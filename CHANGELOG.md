...
- Fixed GPU acceleration issue: Added hardware decode support (`-hwaccel`) for NVIDIA (CUDA), AMD (D3D11VA/VAAPI), and Apple Silicon (VideoToolbox). Previously only hardware encoding was enabled, causing low GPU utilization and high CPU usage during video compression.

Enhancements:

- Improved hardware encoder detection with dedicated helper functions for NVIDIA, AMD, and VideoToolbox encoders.
- Refactored FFmpeg command building to properly place hardware decode arguments before input file.
