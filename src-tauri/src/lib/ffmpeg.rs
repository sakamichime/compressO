...
fn is_nvidia_encoder(codec: &str) -> bool {
    matches!(codec, "h264_nvenc" | "hevc_nvenc")
}

fn is_amd_encoder(codec: &str) -> bool {
    matches!(codec, "h264_amf" | "hevc_amf")
}

fn is_videotoolbox_encoder(codec: &str) -> bool {
    matches!(codec, "h264_videotoolbox" | "hevc_videotoolbox")
}

fn get_hardware_decode_args(codec: &str) -> Vec<String> {
    if is_nvidia_encoder(codec) {
        vec!["-hwaccel".to_string(), "cuda".to_string()]
    } else if is_amd_encoder(codec) {
        if cfg!(target_os = "windows") {
            vec!["-hwaccel".to_string(), "d3d11va".to_string()]
        } else {
            vec!["-hwaccel".to_string(), "vaapi".to_string()]
        }
    } else if is_videotoolbox_encoder(codec) {
...
fn get_hardware_quality_params(codec: &str, quality: u16) -> Vec<String> {
    let quality_value = if (0..=100).contains(&quality) {
        let qp = 51 - (quality as u16 * 51 / 100);
        qp.to_string()
    } else {
        "28".to_string()
    };
    ...
    match codec {
        ...
        "h264_videotoolbox" | "hevc_videotoolbox" => {
            vec!["-q:v".to_string(), quality_value]
        }
        _ => vec![],
    }
}

fn get_software_crf_params(quality: u16) -> String {
    let max_crf: u16 = 36;
    let min_crf: u16 = 24;
...
impl FFMPEG {
    ...
    pub async fn check_encoder_available(app: &AppHandle, encoder: &str) -> bool {
        let shell = match app.shell().sidecar("compresso_ffmpeg") {
            Ok(cmd) => cmd,
            Err(_) => return false,
        };

        let mut command: Command = shell.into();
        command
            .args(["-encoders"])
            .stdout(Stdio::piped())
            .stderr(Stdio::piped());
    ...
    fn get_fallback_encoder(codec: &str, convert_to_extension: &str) -> String {
        match codec {
            "h264_nvenc" | "h264_amf" | "h264_videotoolbox" => {
            ...
            "hevc_nvenc" | "hevc_amf" | "hevc_videotoolbox" => {
                if convert_to_extension == "webm" {
                    "libvpx-vp9".to_string()
                } else {
                    "libx265".to_string()
    ...
    pub async fn compress_video(
        ...
        // Determine codec early to set up hardware decode args before input
        ...
        let final_codec = if is_hardware_encoder(&output_codec) {
            let encoder_available = Self::check_encoder_available(&self.app, &output_codec).await;
            if encoder_available {
                output_codec
            } else {
                log::warn!(
                    "[ffmpeg] Hardware encoder {} not available, falling back to software encoder",
                    output_codec
                );
                Self::get_fallback_encoder(&output_codec, convert_to_extension)
            }
        } else {
            output_codec
        };
        ...
        // Add hardware decode args BEFORE input file for GPU acceleration
        let hw_decode_args = get_hardware_decode_args(&final_codec);
        for arg in hw_decode_args.iter() {
            cmd_args.push(arg.clone());
        }

        cmd_args.push("-i".to_string());
        cmd_args.push(video_path.to_string());
        ...
        if convert_to_extension != "webm" {
            if let Some(thumb_path) = custom_thumbnail_path {
                if thumb_path.len() > 0 {
                    cmd_args.push("-i".to_string());
                    cmd_args.push(thumb_path.to_string());
                    input_index += 1;
                }
            }
        }
        ...
        cmd_args.push("-c:v:0".to_string());
        cmd_args.push(final_codec.clone());

        let mut quality_args_owned: Vec<String> = Vec::new();
        if preset_name.is_some() || (0..=100).contains(&quality) {
            if is_hardware_encoder(&final_codec) {
                quality_args_owned = get_hardware_quality_params(&final_codec, quality);
            } else {
                let crf_value = get_software_crf_params(quality);
        ...
        if let Some(fps_val) = fps {
            ...
            cmd_args.push(fps_val.to_string());
        }
        ...
        cmd_args.extend(audio_args_owned.iter().cloned());

        if audio_config.volume == 0 {
            cmd_args.push("-an".to_string());
        }
        ...
        for arg in metadata_args.iter().cloned() {
            cmd_args.push(arg);
        }
        ...
        cmd_args.extend(subtitle_args_owned.iter().cloned());

        if custom_thumbnail_path.is_some() && convert_to_extension != "webm" {
            if let Some(thumb_path) = custom_thumbnail_path {
                if thumb_path.len() > 0 {
                    cmd_args.push("-c:v:1".to_string());
                    if thumb_path.to_lowercase().ends_with(".webp") {
                        cmd_args.push("png".to_string());
                    } else {
                        cmd_args.push("copy".to_string());
                    }
                    cmd_args.push("-map".to_string());
                    cmd_args.push("1".to_string());
                    cmd_args.push("-disposition:v:1".to_string());
                    cmd_args.push("attached_pic".to_string());
                }
            }
        }
