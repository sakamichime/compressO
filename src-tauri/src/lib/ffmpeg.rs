...
fn is_hardware_encoder(codec: &str) -> bool {
    matches!(
        codec,
        "h264_nvenc" | "hevc_nvenc" | "h264_amf" | "hevc_amf" | "h264_videotoolbox"
            | "hevc_videotoolbox"
    )
}
    ...
    pub async fn compress_video(
        ...
        let final_codec = if is_hardware_encoder(&output_codec) {
            let encoder_available =
                Self::check_encoder_available(&self.app, &output_codec).await;
        ...
        cmd_args.extend_from_slice(&["-c:v:0", final_codec.as_str()]);
